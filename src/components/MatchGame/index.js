...
class MatchGame extends Component {
  ...
  onTimer = () => {
    const {timeOfGame, isGameOver} = this.state
    this.setState(prev => ({timeOfGame: prev.timeOfGame - 1}))
    if (timeOfGame === 1) {
      this.stopTimer()
  ...
  onStartTimer = () => {
    this.clearId = setInterval(this.onTimer, 1000)
  }
  ...
  onCheck = () => {
    const {selectedImageId, activeImageId} = this.state
  ...
  onClickImageItem = imageId => {
    this.setState({activeImageId: imageId}, this.onCheck)
  }
  ...
  renderHome = () => {
    const {activetabid, selectedImage, timeOfGame, score} = this.state
    ...
    return (
      <div className="app-bg-container">
        <Navbar timeOfGame={timeOfGame} score={score} />
        <div className="body-container">
          <div className="selected-image">
            <img className="image" alt="match" src={selectedImage} />
          </div>
  ...
  renderScoreCard = () => {
    const {score, timeOfGame} = this.state
            ...
            <div className="score-card">
              <div className="tropy-container">
                ...
                >
                  ...
                  />
                  <p className="but-text">PLAY AGAIN</p>
                </button>
              </div>
            </div>
          </div>
  ...
  render() {
    ...
    return <> {isGameOver ? this.renderScoreCard() : this.renderHome()} </>
  }
}
